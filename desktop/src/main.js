...
function showOllamaRequired() {
  const dropZone = document.getElementById("drop-zone");
  const existing = document.getElementById("ollama-required-banner");
  if (existing) existing.remove();

  const banner = document.createElement("div");
  banner.id = "ollama-required-banner";
  banner.className = "ollama-required-banner";
  banner.innerHTML = `
    <div class="ollama-required-icon">&#9888;</div>
...
const BEAM_MODELS = [
  {
    id: "torchsight/beam",
    label: "torchsight/beam",
    quant: "q4_K_M",
    size: "~17 GB",
    desc: "Default — recommended for most systems",
  },
  {
    id: "torchsight/beam:q8_0",
    label: "torchsight/beam:q8_0",
    quant: "q8_0",
    size: "~28 GB",
    desc: "Higher accuracy, needs more resources",
  },
  {
    id: "torchsight/beam:f16",
    label: "torchsight/beam:f16",
    quant: "f16",
    size: "~54 GB",
    desc: "Full precision, datacenter GPUs only",
  },
];
...
const VISION_MODELS = [
  {
    ...
    size: "~4.9 GB",
    desc: "Image analysis and description",
  },
];
  ...
  try {
    ...
    beamList.innerHTML = BEAM_MODELS.map((m) => {
      const isInstalled = installed.some(
        (i) =>
          i.startsWith(m.id.split(":")[0]) &&
          (m.id.includes(":") ? i.includes(m.id.split(":")[1]) : true),
      );
    ...
    wireModelSelect(beamList, (model) => {
      selectedBeamModel = model;
    });
    ...
    beamList.innerHTML =
      '<div class="model-select-loading">Could not connect to Ollama</div>';
...
async function showFileList(path) {
  ...
  container.style.display = "block";
  container.innerHTML =
    '<div class="file-list-loading">Scanning directory...</div>';

  try {
    const files = await invoke("list_files", { path });
    if (files.length === 0) {
      container.innerHTML =
        '<div class="file-list-loading">No files found in directory</div>';
      return;
    }
    ...
    const extTags = Object.entries(extCounts)
      ...
      .map(
        ([ext, count]) =>
          `<span class="ext-tag">.${esc(ext)} <span class="ext-count">${count}</span></span>`,
      )
      .join("");
...
async function startScan(path) {
  // Re-check Ollama before scanning
  await checkOllama();
  if (!ollamaConnected) {
    showOllamaRequired();
    return;
...
function renderResults(result) {
  ...
  const flagged = result.files.filter((f) =>
    f.findings.some((fn) => fn.severity !== "info" && fn.category !== "safe"),
  );
  const clean = result.files.filter(
    (f) =>
      !f.findings.some(
        (fn) => fn.severity !== "info" && fn.category !== "safe",
      ),
  );
...
function setupSettings() {
  ...
  document
    .getElementById("install-ollama-btn")
    .addEventListener("click", async () => {
      try {
