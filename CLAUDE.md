...
│   ├── data/
│   └── output/
├── desktop/       # Tauri v2 desktop app (Rust + HTML/JS)
│   ├── src-tauri/
│   └── src/
├── site/          # Documentation website (Astro)
├── report/        # PDF/HTML report generator (Python)
└── Cargo.toml     # Workspace root
```

## Architecture
- **Rust CLI** with interactive REPL (`core/src/cli/repl.rs`)
- **Beam model** (`torchsight/beam`) — custom LoRA fine-tune of Qwen 3.5 27B, uses `/api/generate` endpoint
...
- **Text pipeline**: read file (or pdftotext for PDFs) → truncate to 6000 chars → beam generate
- **Regex safety net** — 35 compiled patterns as fallback for attacks the LLM misses
- **Desktop app** — Tauri v2 (Rust backend + HTML/JS frontend), calls CLI for scanning

## Key Files
- `core/src/main.rs` — CLI args, subcommands, health checks, entry point
- `core/src/config.rs` — `.torchsight.toml` config file loader
- `core/src/analyzers/text.rs` — text/PDF analysis, `parse_beam_findings()` parser
- `core/src/analyzers/image.rs` — OCR + vision + beam hybrid pipeline
- `core/src/llm/ollama.rs` — Ollama HTTP client (600s timeouts, temperature=0, auto-pull)
- `core/src/scanner/pipeline.rs` — file discovery, progress bar, error handling
- `core/src/scanner/discovery.rs` — file discovery, `.torchsightignore` support
- `core/src/cli/git_hook.rs` — pre-commit hook install/uninstall/scan
- `core/src/cli/watch.rs` — real-time file system watcher
- `core/src/cli/stdin.rs` — stdin pipe and git diff scanning
- `core/src/cli/policy.rs` — YAML policy engine (block/warn/ignore rules)
- `core/src/report/` — JSON, Markdown, Terminal, PDF, SARIF, HTML output
- `report/generate.py` — PDF report generator (called via `uv run`)
- `beam/output/Modelfile` — Ollama model definition (system prompt must match training)
- `beam/scripts/sft_converter.py` — SYSTEM_PROMPT is the source of truth
- `desktop/src-tauri/src/lib.rs` — Tauri commands (scan, models, stats, PDF export)
- `desktop/src/main.js` — Desktop UI logic
...
- GGUFs in `beam/output/`: `beam-1.0-q4_K_M.gguf` (~17GB), `beam-1.0-q8_0.gguf` (~28GB)
...
- Inference: temperature=0 for deterministic classification
...
- PDF reports generated via Python (`uv run`), requires `uv` + weasyprint system deps
- Beam model sometimes generates repetitive filler after valid findings — parser recovers truncated JSON
- `parse_beam_findings` deduplicates by category:subcategory and skips "safe" entries when non-safe findings exist
- Desktop PDF export needs weasyprint system deps (see issue #5)
